...
files.forEach(filename => {
  ...
  const reg = /https:\/\/github.com\/rdd-giga\/qlear-issues\/wiki\/(.*?)\)/g;
  let file = fs.readFileSync(path.join(__dirname, '../qlear-issues.wiki/', filename), 'utf8').replace(reg, (match, p1, offset, string) => `/${p1.replace(/%3F/g, '')})`);

  const name = filename.replace(/\[\w+\]\-/g, '').replace(/-/g, ' ');
  const findFunc = v => ~v.indexOf(encodeURIComponent(filename.slice(0, -3))) || ~v.indexOf(filename.slice(0, -3))
  const index = content.findIndex(findFunc);
  if (filename === 'Tongdy-MSD-1628-(with-WiFi).md') {

  }
  let subTitle = null;
  let sortIndex = -1;
  if (~index) {
    let group = content[index].replace(/%3F/g, '?').match(/(\s*-\s\[[\s\w\.\?\&\!\-\(\)]*\]\(\/[\w\-\:\?\.\&\(\)]*\))/g);
    sortIndex = group.findIndex(findFunc);
    const item = group.find(findFunc);
    const length = item && item.match(/\s+\-/)[0].length;

    if (length === 6) {
      const index = group.findIndex(findFunc);
  ...
  fs.writeFileSync(path.join(resultPath, filename.replace('?', '')), templet(name.slice(0, -3), file, sideTitle[index], subTitle, sortIndex));
});
